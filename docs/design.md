...
A connection whose account went live needs nothing from the user, so it
leaves `pending` (mirroring `mcp-server.ts:652-660`) and is kept in
`resumable`: it is still the card a resume acts on, and its siblings decide
whether that resume is allowed.
