...
| `report` | `[--md] [--check-042] [--run <ref>\|last] [--no-tests] [--close\|--no-close]` | one run: `--run` resolves an open run first, then the history; turns and tokens from **every run thread's** `events/<thread>.ndjson`, with the turns on a thread another run also records allocated by this run's open-delegation windows — a queued delegation's turn must start inside its window, while an ask converted to one was already running (`index.ts:7894-7917`) — and the rest counted as `shared`; outcomes for the run, lead text, decisions, `git log <sentSha>..HEAD`, worktree list; verifies the record step; runs the project test command independently; `--check-042` adds the pack-validation checks (below); `--md` renders the evidence section; the record commit and the task-log entry must name the run (its slug, title or bead, matched as a whole word so T1 cannot take T10's) and fall inside its window: a dated heading is claimable only by the runs whose window contains it, and among those the entry belongs to the one it names before any other; a name two dispatches share is attributed only when the section names something of this run's that the twin does not have and nothing of the twin's that this run does not have. The merged commit is the one the closing text asserts for this run, in a clause no rival shares, else the one qualifying segment of a `docs(team)` record commit's subject — the same segment rule that decides whether a commit is this run's record at all. A twin the window excluded still counts in the log, where both dispatches write; in this run's own closing text and record commit it counts only through an identifier the two do not share; a task log that cannot be read leaves the record unknown rather than passing on a commit that merely touched its path; the root check ignores the other open runs' worktrees and branches, and checks the whole repository again when the last run closes; **closes the run**: moves it out of `runs` into `history` by `runId` with a result of `passed`, `incomplete`, or `failed`, and refuses when another open run closed meanwhile, since the repository check treated its worktree and branch as owned | data dir, git, `bd show` |
