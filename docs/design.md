...
| `bind` | `[--default eng/model[/effort]] [--reviewers eng/model/effort] [--model "<bot>=…"]… [--approval auto\|ask] [--no-room]` | launcher-level idle check first; room `cwd` only when it differs (a pinned room rejects even an equal `cwd`, `index.ts:9566`); bot `cwd`; model only when it differs (equal selections pass while busy, `index.ts:1050-1057`, but a pending approval grant rejects any model PATCH, `index.ts:9930`); approval mode (skipped with a note for grok bots); `.worktrees/` and `.omb/` into `.git/info/exclude`; re-reads the roster; reports partial results per bot; exit 3 on any 409 | `PATCH /api/groups/:id`, `/api/bots/:id`, `/api/bots/:id/model` |
