...
- **A stopped task keeps its worktree on purpose.** After a discard, two failed rounds, a BLOCKED implementer, or a failing default branch, `.worktrees/<slug>` and `task/<slug>` stay so someone can look, and the report says so; `reconcile` then fails and the next `task` refuses to dispatch. An **open** run's own `.worktrees/<slug>` on `task/<slug>` is not a leftover: `reconcile` matches that exact pair to the run and reports its owner. Anything else has no owner and blocks the next dispatch until it is removed or given to a run with `reconcile --claim <slug> --run <ref>` — one the lead created under another name, one of the right name in another directory, and `.worktrees/<slug>` checked out on some other branch.
