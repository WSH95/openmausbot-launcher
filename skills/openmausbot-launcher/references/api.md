...
| `openmausbot-server.lease` | The exclusive lease, taken before any identity, session, configuration or store state is loaded (`index.ts:335-339`) and released at exit (`index.ts:340-352`). The record is `{version, pid, host, token, createdAt}` (`electron/data-dir-lease.mjs:294-300`): the pid is the server process that answers `/api/health`. A second server on the directory is refused with `OpenMausBot is already using this data directory (process <pid>). Close the other instance first.` for a live owner on this host, or `This OpenMausBot data directory is already owned by a process on another machine. Lease record: …` for another host (`:323-331`), and the supervisor forwards that failure (`cli.ts:480`). Liveness is `kill(pid, 0)`, where `EPERM` counts as alive (`:82-92`); a record that is not the shape above is refused, not retired (`:24-41,66-72`); only a dead owner's record is retired, by one contender at a time (`:137-180`), and a release happens only for the record this process wrote (`:346-359`). **The launcher never acquires, reads, repairs or deletes this file**; exclusion and stale recovery are upstream's, they decide by hostname and pid liveness, and they do not cover shared volumes or pid namespaces. A recovery error can name a file whose name carries a token (`:123-126,159`), so nothing after `openmausbot-server.lease` is ever printed. |
