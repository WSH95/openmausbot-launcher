...
export function brief(ev, snap, task, now = Date.now()) {
  ...
  switch (ev.state) {
    ...
    case "needs-user": {
      ...
      // A settled card in resumable accepts only --resume; neither --provide
      // nor --dismiss can select it (index.ts:6767-6773, 12208-12222).
      if (p?.kind === "secret") {
        const settled = p.secret?.provided || p.secret?.dismissed;
        return `${slug} · CREDENTIAL · ${p.botName} needs the ${p.secret?.label ?? p.secret?.target} → omb answer ${settled ? `--resume --request ${id}` : `--provide --secret-stdin --request ${id} < <file the user wrote> | --dismiss`}`;
      }
